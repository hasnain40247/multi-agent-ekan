...
from mpe.multiagent.environment import MultiAgentEnv, MultiDiscrete
from mpe.multiagent.environment_vec import MultiAgentEnv as MultiAgentEnvVec
import mpe.multiagent.scenarios as scenarios
import numpy as np

def sample_action_n(action_spaces):
    action_n = []

    for action_space in action_spaces:
        if isinstance(action_space, spaces.box.Box):
            action_n.append(action_space.sample()) 
        else:
...
def gen_action_range(action_spaces):
    action_range = None
    for action_space in action_spaces:
        if isinstance(action_space, spaces.box.Box):
            low_range =  np.max(np.abs(action_space.low))
            high_range = np.max(np.abs(action_space.high))
            if action_range is None:
                action_range = np.amax((low_range, high_range))
            else:
                action_range = np.amax((action_range, low_range, high_range))
        else:
            action_range = None
    return action_range

def gen_n_actions(action_spaces):
    """
    :param action_space: list
    :return: n_action: list
    """
    ...
    for action_space in action_spaces:
        if isinstance(action_space, spaces.discrete.Discrete):
            ...
            for h, l in zip(action_space.high, action_space.low):
                ...
                one_agent_n_action += int(h - l + 1)
            n_actions.append(one_agent_n_action)
        elif isinstance(action_space, spaces.box.Box):
            n_actions.append(action_space.shape[0])
        else:
            raise NotImplementedError
    return n_actions
def make_env(scenario_name, continuous, arglist, benchmark=False):
    ...
    # create multiagent environment
    if benchmark:
        env = MultiAgentEnv(scenario, world, continuous = continuous,
                            reset_callback = scenario.reset_world,
                            state_callback = scenario.state,
                            reward_callback = scenario.reward, 
                            observation_callback = scenario.observation, 
                            info_callback = scenario.benchmark_data,
                            seed_callback=scenario.seed, 
...
def make_env_vec(scenario_name, arglist, benchmark=False):
    ...
    # create multiagent environment
    if benchmark:
        env = MultiAgentEnvVec(world, scenario.reset_world, scenario.reward, scenario.observation, scenario.benchmark_data,
                               seed_callback=scenario.seed, cam_range=scenario.world_radius)
    else:
        env = MultiAgentEnvVec(world, scenario.reset_world, scenario.reward, scenario.observation,
                               seed_callback=scenario.seed, cam_range=scenario.world_radius)
    return env
