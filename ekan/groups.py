...
from .utils import Kronsum, consistent_hash
...
class Group(object, metaclass=Named):
    ...
    def __init__(self, *args, **kwargs):
        if self.d is NotImplemented:
            ...
            if self.discrete_generators is not NotImplemented and len(
        ...
        if self.is_orthogonal is None:
            ...
            if len(self.lie_algebra) != 0:
                ...
                self.is_orthogonal &= (
                    rel_err(-A_dense.transpose((0, 2, 1)), A_dense) < 1e-6
                )
            if len(self.discrete_generators) != 0:
                h_dense = np.stack(
                    [hi @ np.eye(self.d) for hi in self.discrete_generators]
                )
                self.is_orthogonal &= (
                    rel_err(h_dense.transpose((0, 2, 1)) @ h_dense, np.eye(self.d))
                    < 1e-6
                )
    ...
    def samples(self, N):
        A_dense = (
            np.stack([Ai @ np.eye(self.d) for Ai in self.lie_algebra])
            if len(self.lie_algebra)
            else np.zeros((0, self.d, self.d))
        )
        h_dense = (
            np.stack([hi @ np.eye(self.d) for hi in self.discrete_generators])
...
class C(Group):
    def __init__(self, k):
        ...
        self.discrete_generators[0, :, :] = np.array(
...
class Cube(Group):
    def __init__(self):
        ...
        self.discrete_generators = np.array(
            [np.eye(6)[perm] for perm in [Fperm, Lperm]]
        )
        super().__init__()
...
class RubiksCube(Group):
    def __init__(self):
        ...
        self.discrete_generators = np.array(
            [np.eye(48)[perm] for perm in [Uperm, Fperm, Rperm, Bperm, Lperm, Dperm]]
        )
        super().__init__()
...
class DirectProduct(Group):
    def __init__(self, G1, G2):
        ...
        self.lie_algebra = np.array(
            [Kronsum([A1, 0 * I2]) for A1 in G1.lie_algebra]
            + [Kronsum([0 * I1, A2]) for A2 in G2.lie_algebra]
        )
        self.discrete_generators = np.array(
            [np.kron(M1, I2) for M1 in G1.discrete_generators]
            + [np.kron(I1, M2) for M2 in G2.discrete_generators]
        )
