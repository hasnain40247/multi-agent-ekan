...
class EKANLayer(nn.Module):
    def __init__(
        ...
        Pwi_terms, multiplicities, perm, invperm = rep_Wi.equivariant_projector(
            lazy=True
        )
        Pwi_terms = [
            torch.tensor(Pwi_term, dtype=torch.float32, device=device)
            for Pwi_term in Pwi_terms
        ]
        ...
        self.Pb = torch.tensor(
            gated_rep_out.equivariant_projector(), dtype=torch.float32, device=device
        )
    ...
    def weight_proj(self):
        w_reshape = (
            self.linear.weight.reshape(
                self.gated_out_dim, self.num + self.k + 1, self.in_dim
            )
            .permute(0, 2, 1)
            .reshape(self.gated_out_dim * self.in_dim, self.num + self.k + 1)
        )
        W_reshape = self.Pwi(w_reshape)
        W = (
            W_reshape.reshape(self.gated_out_dim, self.in_dim, self.num + self.k + 1)
            .permute(0, 2, 1)
            .reshape(self.gated_out_dim, (self.num + self.k + 1) * self.in_dim)
        )
        return W
    ...
    def forward(self, x):
        ...
        gate_scalars_sp = B_batch(
            x=gate_scalars, grid=self.grid, k=self.k, extend=True, device=self.device
        ).permute(2, 1, 0)
        ...
        activations = torch.einsum(
            "ijk,ik->ijk", gate_scalars, x[..., : self.in_dim]
        ).reshape(batch, -1)
        W = self.weight_proj()
        ...
        out = 0.1 * (bi_W @ out[..., None])[..., 0] + out
        return out

    def weight_proj_inv(self, W):
        W_reshape = (
            W.reshape(self.gated_out_dim, self.num + self.k + 1, self.in_dim)
            .permute(0, 2, 1)
            .reshape(self.gated_out_dim * self.in_dim, self.num + self.k + 1)
        )
    ...
    def update_grid_from_samples(self, x):
        ...
        gate_scalars_sp_update = B_batch(
            x=gate_scalars_update,
            grid=self.grid,
            k=self.k,
            extend=True,
            device=self.device,
        ).permute(2, 1, 0)
        gate_scalars_sb_update = (
            self.base_fun(gate_scalars_update).permute(1, 0).unsqueeze(dim=1)
        )
        gate_scalars_update = torch.cat(
            (gate_scalars_sp_update, gate_scalars_sb_update), dim=1
        )
        gate_scalars_update = gate_scalars_update[..., self.indices]
        activations_update = torch.einsum(
            "ijk,ik->ijk", gate_scalars_update, x[..., : self.in_dim]
        ).reshape(batch, -1)
