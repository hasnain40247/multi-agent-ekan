...
def create_dataset_randn(
    f, n_var=2, range=1.0, train_num=1000, test_num=1000, device="cpu", seed=0
):
...
def Kronsum(Ms):
    result = np.zeros(1)
    for M in Ms:
        result = np.kron(result, np.eye(M.shape[0])) + np.kron(
            np.eye(result.shape[0]), M
        )
    return result
...
def visualization(results, name, folder="./results", classify=False):
    plt.plot(
        np.arange(len(results["train_loss"])), results["train_loss"], label="train"
    )
