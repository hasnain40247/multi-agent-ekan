...
from .utils import Kron, Kronsum, DirectSum, consistent_hash
...
class Rep(object):
    ...
    def constraint_matrix(self):
        ...
        constraints = []
        constraints.extend(
            [self.rho(h) - np.eye(n) for h in self.G.discrete_generators]
        )
...
def bilinear_weights(out_rep, in_rep):
    ...
    active_dims = sum(
        [
            W_multiplicities.get(rep, 0) * nelems(n, rep)
    ...
    if isinstance(x_rep, ScalarRep):
        ...
        reduced_indices_dict = {
            rep: ids[np.random.choice(len(ids), nelems(len(ids), rep))].reshape(-1)
...
class SumRep(Rep):
    def __init__(self, *reps, extra_perm=None):
        reps = [
            SumRepFromCollection({Scalar: rep}) if isinstance(rep, int) else rep
            for rep in reps
        ]
    ...
    def T(self):
        return SumRep(
            *[rep.T for rep, c in self.reps.items() for _ in range(c)],
    ...
    def __str__(self):
        tensors = "+".join(
            f"{count if count > 1 else ''}{rep}" for rep, count in self.reps.items()
        )
        return tensors
    ...
    def __call__(self, G):
        return SumRepFromCollection(
            {rep(G): c for rep, c in self.reps.items()}, perm=self.perm
        )
...
def mul_reps(ra, rb):
    ...
    if (
        (isinstance(ra, SumRep) and isinstance(rb, Rep))
        or (isinstance(ra, Rep) and isinstance(rb, SumRep))
        or (isinstance(ra, SumRep) and isinstance(rb, SumRep))
    ):
...
def distribute_product(reps, extra_perm=None):
    ...
    reps = [
        rep if isinstance(rep, SumRep) else SumRepFromCollection({rep: 1})
        for rep in reps
    ]
    ...
    for prod in itertools.product(*[rep.reps.items() for rep in reps]):
        rs, cs = zip(*prod)
        prod_rep, canonicalizing_perm = (
            product(cs) * reduce(lambda a, b: a * b, rs)
        ).canonicalize()
        ordered_reps.append(prod_rep)
        shape = []
        for r, c in prod:
            shape.extend([c, r.size()])
        axis_perm = np.concatenate(
            [2 * np.arange(len(prod)), 2 * np.arange(len(prod)) + 1]
        )
        mul_perm = (
            np.arange(len(canonicalizing_perm))
            .reshape(shape)
            .transpose(axis_perm)
            .reshape(-1)
        )
...
def rep_permutation(repsizes_all):
    size_cumsums = [
        np.cumsum([0] + [size for size in repsizes]) for repsizes in repsizes_all
    ]
...
class ProductRep(Rep):
    def __init__(self, *reps, extra_perm=None, counter=None):
        if counter is not None:
            ...
            reps, perms = zip(*[rep.canonicalize() for rep in reps])
            rep_counters = [
                rep.reps if type(rep) == ProductRep else {rep: 1} for rep in reps
            ]
            self.reps, perm = self.compute_canonical(rep_counters, perms)
            self.perm = extra_perm[perm] if extra_perm is not None else perm
    ...
    def __eq__(self, other):
        return (
            isinstance(other, ProductRep)
            and self.reps == other.reps
            and (self.perm == other.perm).all()
        )
...
class DirectProduct(ProductRep):
    def __init__(self, *reps, counter=None, extra_perm=None):
        if counter is not None:
            self.reps = counter
            self.reps, perm = self.compute_canonical(
                [counter], [np.arange(self.size())]
            )
            self.perm = extra_perm[perm] if extra_perm is not None else perm
        else:
            reps, perms = zip(*[rep.canonicalize() for rep in reps])
            rep_counters = [
                rep.reps if type(rep) == DirectProduct else {rep: 1} for rep in reps
            ]
            ...
            self.reps, perm2 = self.compute_canonical(
                [counter], [np.arange(self.size())]
            )
            self.perm = (
                extra_perm[perm[perm2]] if extra_perm is not None else perm[perm2]
            )
        self.invperm = np.argsort(self.perm)
        self.canonical = (self.perm == self.invperm).all()
        self.is_permutation = all(rep.is_permutation for rep in self.reps.keys())
...
class DeferredProductRep(Rep):
    def __init__(self, *reps):
        ...
        for rep in reps:
            ...
            self.to_prod.extend(
                rep.to_prod if isinstance(rep, DeferredProductRep) else [rep]
            )
...
def lazy_Pinv(P_terms, multiplicities, perm, invperm, v):
    ...
    for P_term, multiplicity in zip(P_terms, multiplicities):
        ...
        elems = (
            torch.linalg.pinv(P_term)
            @ v[i:i_end].T.reshape(k * multiplicity, P_term.shape[0]).T
        )
