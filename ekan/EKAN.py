...
import numpy as np
from .EKANLayer import EKANLayer
from .LBFGS import *
import os
import glob
...
class EKAN(nn.Module):
    def __init__(
        ...
        self.G = group
        middle_layers = [
            (c(group) if isinstance(c, Rep) else uniform_rep(c, group)) for c in width
        ]
        ...
        Pw0_terms, multiplicities, perm, invperm = rep_W0.equivariant_projector(
            lazy=True
        )
        Pw0_terms = [
            torch.tensor(Pw0_term, dtype=torch.float32, device=device)
            for Pw0_term in Pw0_terms
        ]
        self.Pw0 = lambda w: lazy_P(
            Pw0_terms, multiplicities, perm, invperm, w.unsqueeze(1)
        )
        self.Pb0 = torch.tensor(
            gated_rep_in.equivariant_projector(), dtype=torch.float32, device=device
        )
    ...
    def train(
        ...
        def reg():
            ...
            for i in range(self.depth):
                ...
                reg_ += torch.sum(
                    torch.abs(self.act_fun[i].Pb @ self.act_fun[i].linear.bias)
                )
            return reg_
        ...
        if loss_fn == None:
            if self.classify:
                epsilon = 1e-7
                loss_fn = lambda x, y: -torch.mean(
                    y.unsqueeze(1) * torch.log(torch.clamp(x, epsilon, 1 - epsilon))
                    + (1 - y.unsqueeze(1))
                    * torch.log(1 - torch.clamp(x, epsilon, 1 - epsilon))
                )
                loss_fn_eval = lambda x, y: torch.mean(
                    y.unsqueeze(1) * torch.round(x)
                    + (1 - y.unsqueeze(1)) * (1 - torch.round(x))
                )
            else:
        ...
        with torch.no_grad():
            ...
            for i in range(0, dataset["test_input"].shape[0], batch_size_test):
                test_id = np.arange(
                    i, min(i + batch_size_test, dataset["test_input"].shape[0])
                )
                overall_test_loss += len(test_id) * loss_fn_eval(
                    self.forward(dataset["test_input"][test_id].to(device)),
                    dataset["test_label"][test_id].to(device),
                )
