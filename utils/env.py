...
def get_env_info(env_name, max_steps=25, seed=0, apply_padding=True):
    ...
    n_agents = env_single.agents
    print("agents:")
    print(n_agents)
    original_action_sizes = [env_single.action_space(agent).shape[0] for agent in n_agents]
    actionsss = [env_single.action_space(agent) for agent in n_agents]

    print(actionsss)
    n_actions = original_action_sizes

    
    print("obs")
    # obs_dims = [env_single.observation_space[i].shape[0] for i in range(len(n_agents))]
    agents = env_single.agents

    obs_dims = [env_single.observation_space(agent).shape[0] for agent in agents]
    obs_dims.insert(0, 0)
    print("sume")
    print(np.sum(obs_dims))
    print("multiplication")
    print(n_actions[0] * len(n_agents))
