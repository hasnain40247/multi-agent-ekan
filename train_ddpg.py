"""
Training script for DDPG with PettingZoo
"""
import torch
import numpy as np
import os
...
from utils.utils import  evaluate_ddpg
...
def parse_args():
    ...
    parser.add_argument("--actor-lr", type=float, default=1e-2, help="Actor learning rate")
    parser.add_argument("--critic-lr", type=float, default=1e-2, help="Critic learning rate")
    ...
    parser.add_argument("--noise-scale", type=float, default=0.2, help="Initial noise scale")
...
def train(args):
    ...
    buffer = ReplayBuffer(
        buffer_size=min(args.buffer_size, args.total_timesteps),
    ...
    evaluate_ddpg(env_evaluate, ddpg_agents, logger, record_gif=args.create_gif, num_eval_episodes=10, global_step=0)
    ...
    for global_step in tqdm(range(1, args.total_timesteps + 1), desc="Training"):
        ...
        actions_list = [ddpg_agents[i].act(states_list[i], add_noise=True, noise_scale=noise_scale) 
                for i in range(num_agents)]
        ...
        if len(buffer) > args.batch_size and global_step % args.update_every == 0:
            for i in range(len(agents)):
                ...
                (states, actions, rewards, next_states, dones, _, _, _) = experiences
                agent_experiences = (states[i], actions[i], rewards[i], next_states[i], dones[i])
                critic_loss, actor_loss = ddpg_agents[i].learn(agent_experiences, args.gamma)
        ...
        if global_step % args.eval_interval == 0 or global_step == args.total_timesteps:
            for i in range(len(agents)):
                ddpg_agents[i].save(logger.dir_name, i)
            evaluate_ddpg(env_evaluate, ddpg_agents, logger,
