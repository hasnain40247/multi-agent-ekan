import numpy as np
import random
from mpe.multiagent.core_vec import World, Agent, Landmark
...
class Scenario(BaseScenario):
    ...
    #     # we have the location of the current agent which is self.pos
    #     # 
    #     #            
    #     loc_others =  batch_obs[:,4 + 2*n_landmarks : ].reshape((batch_size, -1, 2))
    #     # we have the locations of the other agents
    #     loc_others = loc_others + loc_agent # loc_others in obs is relative to loc_agent

    #     # we take it and make it absolute - needed for the graph
